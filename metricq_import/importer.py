...
class FakeAgent(Client):
    async def connect(self):
        ...
        logger.warn(
            "Sending db.register to manager, the response may take some time. Stay a while and listen..."
        )
...
class DataheapToHTAImporter(object):
    def __init__(
        ...
        self._couchdb_client = cloudant.client.CouchDB(
    ...
    def real_run(self):
        ...
        self._confirm(
            f"Please make sure the MetricQ db with the token "
    ...
    def dry_run(self):
        mysql = pymysql.connect(
            ...
            port=self._import_port,
            user=self._import_user,
            passwd=self._import_password,
        ...
        for metric in self._metrics:
            with mysql.cursor() as cursor:
                ...
                t_max /= 1e3
                click.echo(
                    f"{em(metric.metricq_name):30s} <== {metric.import_name:30s} with {count:14,} entries"
                )
                click.echo(
                    f"        intervals {metric.interval_min:,}, {metric.interval_max:,}, {metric.interval_factor}"
                )
                ...
                click.echo(
                    f"        time range {dt_min} - {dt_max}, avg interval {interval_avg}"
                )
                delta = datetime.datetime.now() - dt_max
                if self._check_max_age and not (
                    datetime.timedelta() < delta < self._check_max_age
                ):
                ...
                tolerance = 1.5
                if self._check_interval and not (
                    expected_interval / tolerance
                    < interval_avg
                    < expected_interval * tolerance
                ):
                    click.secho(
                        f"suspicious interval, expected {expected_interval}",
                        bg="red",
                        bold=True,
                    )
                    self._confirm()
                ...
                if self._check_values:
                    cursor.execute(
                        f"SELECT MIN(value) as value_min, MAX(value) as value_max"
                        f" FROM `{metric.import_name}`"
                    )
    ...
    def _update_config(self):
        config_metrics = {
            metric.metricq_name: metric.config for metric in self._metrics
        }
        ...
        try:
            ...
            raise KeyError(
                f"Please add a basic configuration in the CouchDB for the db '{self._metricq_token}'"
            )
        ...
        if not self._resume:
            ...
            conflicting_metrics = [
                metric
                for metric in self._metrics
                if metric.metricq_name in current_metric_names
            ]

            if conflicting_metrics:
                print(
                    "The following metrics have already been defined in the database, but are in the import set:"
                )
                ...
                if not self._confirm("Overwrite old entries?", abort=False):
                    raise RuntimeError(
                        "Please remove the config for the existing metrics"
    ...
    def _run_import(self):
        ...
        with click.progressbar(
            length=self.num_import_metrics,
            label="Importing metrics",
            item_show_func=self._last_completed_metric_name,
        ) as bar:
    ...
    async def import_metric(self, metric):
        if self._resume:
            try:
                ...
                if old_import.get("return_code", -1) != 0:
                    click.echo(
                        f"{metric.metricq_name} was partially imported, please cleanup"
                    )
        ...
        conffile, conffile_name = tempfile.mkstemp(
            prefix="metricq-import-", suffix=".json", text=True
        )
        ...
        args = (
            ...
            conffile_name,
            "--max-timestamp",
            str(int(self._import_begin.posix_ms)),
        )
